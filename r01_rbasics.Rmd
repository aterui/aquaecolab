...
We will use `iris`, the built-in data set in R, for this exercise.
...
**Symbol color (border)**\
`col` argument (double quote `"color name"` when specifying). Google **r color name** for more options.
...
**Symbol color (fill)**\
`bg` argument (double quote `"color name"` when specifying). Available for a subset of symbol options (some symbols have pre-defined filled color).
...
**Label**\
`ylab` or `xlab` arguments. Provide `"double-quoted text"`.
...
**Box width**\
`width` argument. Provide a vector with the number of factor levels (three in this case as there are three species on x-axis).
...
boxplot(Sepal.Length ~ Species, data = iris,
        col = "lightgray", border = "gray",
        width = rep(0.5, 3), ylab = "Sepal length (cm)",
        axes = F)
box(bty = "l")
