...
This script creates a vector of numbers composed of {10.1, 5.3, 8.8, 6.7, 8.2}. `x` is a ‘box’ where the vector information is stored, and ‘<-’ is the arrow that commands R to store {10.1, 5.3, 8.8, 6.7, 8.2} in a box `x`. `x` is refered to as an **object**. Let’s see what’s in `x`:
...
R can handle arithmatic opeartions between objects. Prepare objects `x` and `y`:
...
You can use objects intead of directly entering numbers. R automatically performs arithmatic operations for each element. For example, enter `y-x`:
...
To answer the following questions, download a <a href = "file/report01_temp.R" download>template</a> (R script file).
...
a. Create a dataframe of 3 variables with 10 elements (name variables as `x`, `y` and `z`. `x` must be `character` while `y` and `z` must be `numeric`.
