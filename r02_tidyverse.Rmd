...
Data manipulation is a critical component of data analysis. You can use spreadsheets or similar to manage your data. However, this form of data manipulation/manipulation should be limited to the process of creating the primary data set. There are several reasons for this. **First and foremost, spreadsheets do NOT keep records of your edits.** If you make any mistake during the process of data manipulation, it will be unnoticed and cannot be recovered for most cases. **Second, spreadsheets are simply unsuitable for data manipulation.** You may need to iterate the same manipulation multiple times - you will make errors sooner or later.
...
Below, I will introduce some useful 'basic' functions, followed by guidance on how to combine them via pipe `%>%`. Before proceeding, let's check the data set we will use for this exercise. R provides a nice data set `iris` , which is well suited for this type of exercise. You can see an overview of the data frame using `View()` function:
...
Also, you can extract variables (column names) as follows:
...
You may need to "filter" rows based on some criteria. `filter()` can help you on this. Let's select rows that belong to one species. For character & factor variables, you will need to double-quote them:
...
Combined conditional expression is also possible:

```{r}

# Species equals "setosa" AND Petal.Length larger than 1.8

filter(iris, Species == "setosa" & Petal.Length > 1.8)

# Species equals "setosa" OR Petal.Length larger than 5.0

filter(iris, Species == "setosa" | Petal.Length > 5)
```

### Exercise

-   Select rows with species `setosa` and `virginica`
-   Select rows with species other than `setosa`
...
Sometimes, you want to add new columns to the data frame. We can add new columns with the same number of elements (i.e., number of rows). We use `mutate()`:
...
This is a very basic example - you can tweak and combine functions in different ways to achieve your data manipulation more effectively.
